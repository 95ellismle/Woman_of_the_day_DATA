...
class GetPage(object):
    ...
    Inputs:
        * url           ->  The url to download or filepath to a HTML file.
        * must_contain  ->  Any strings the website must have.
        * soup          ->  The soup object from the webpage
        * throw_error   ->  Throw an error if things go wrong
        * do_soup       ->  Parse the page with the 'lxml' bs4 parser.
    ...
    """
    def __init__(self, url, must_contain=[], soup=False, throw_error=True,
                 do_soup=True):
        self.bad_site = False
        self.url = url
        self.no_url = True
        ...
        if not self.soup and not isFile:
            self.no_url = False              
            self._get(url)
            ...
            with open(url, 'r') as f:
                self.fileTxt = f.read()
                if do_soup:
                    self.soup = bs4.BeautifulSoup(self.fileTxt, 'lxml')
...
class GetImage(GetPage):
    """
    Will download an image object using requests.
    ...
    def __init__(self, url, throw_error=True):
        self.url = url
        self.error = throw_error
        self._dwnld_obj(url)
    
    def _dwnld_obj(self, url):
        """
        Will download the image object and save it as self.R        
        ...
        Returns
        -------
        None. (sets variable self.R)

        """
        self.R = rq.get(url, stream=True)
        if self.R.status_code != 200:
            self._exit("Can't find image!")
    
    def save(self, filename):
        """
        Will save the image to a file.

        Parameters
        ----------
        filename : str
            The location the file should be saved.
